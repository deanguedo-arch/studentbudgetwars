...
class StatusBar(tk.LabelFrame):
    def __init__(self, master: tk.Misc):
        ...
        self.label = tk.Label(self, anchor="w", justify="left", bg="#c0c0c0", font=("Consolas", 11, "bold"))
    ...
    def render(self, state: GameState, bundle: ContentBundle) -> None:
        ...
        status_text = (
            ...
            f"Social {player.social_stability:>3}/{state.max_social_stability}"
        )
        self.label.configure(text=status_text)

    def set_large_text(self, enabled: bool) -> None:
        self.label.configure(font=self._large_font if enabled else self._normal_font)
